...
let fgmaskWidth: number = 200;
let fgmaskHeight: number = 150;
let now: number = 0;
let camera = {
    left: -1,
    right: 1,
...
class Particle {
    ...
    public animate() {
        const pixelX = Math.floor(map(this.position[0], camera.left, camera.right, 0, fgmaskWidth));
        const pixelY = Math.floor(map(this.position[1], camera.top, camera.bottom, 0, fgmaskHeight));
...
const particles = new Array(NUM_PARTICLES).fill(null).map((_, idx) => {
...
self.addEventListener("message", (e) => {
    ...
    if (message.type === "foregroundUpdate") {
        ...
        fgmaskWidth = message.fgmaskWidth;
        fgmaskHeight = message.fgmaskHeight;
        now = message.now;
        animate();
    }
});
