...
export class LeafTemplate {
    ...
    private static addVerticesAndSkinningAtVeinPositions(leaf: VeinedLeaf, geometry: THREE.Geometry) {
        for (let i = 0; i < leaf.world.length; i++) {
            ...
            const yFray = Math.pow(cost, 8) * 0.04 * Math.cos(vein.distanceToRoot / 2 / vein.depth);
            const y = yCup + yLift + yNoise + yFray;
            // const y = 0;
            geometry.vertices.push(new THREE.Vector3(x, y, z));
            // bone array goes like this:
            // [baseBone, forwardBones x5, sideBone]
            const fractionalIndex = x * LEAF_NUM_BONES;
            const prevIndex = Math.floor(fractionalIndex);
            const nextIndex = Math.ceil(fractionalIndex);
            const t = fractionalIndex - prevIndex;

            console.log(prevIndex, nextIndex, fractionalIndex, t);

            const orthoIndex = LEAF_NUM_BONES + 1;

            const orthoWeight = 0; // Math.abs(z) / 10.;
            // const orthoWeight = 0;
